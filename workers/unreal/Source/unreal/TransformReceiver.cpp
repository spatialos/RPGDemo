    ...
    const worker::ComponentUpdateOp<improbable::corelibrary::transforms::TransformState>& op)
{
  ...
  if (!loc.empty())
  {
    ...
    UE_LOG(LogTemp, Warning, TEXT("UTransformReceiver: Received actor (%s) position update (%s)"),
           *GetOwner()->GetName(), *mLocation.ToString())
  }
  if (!rot.empty())
  {
    mRotation = ToUnrealQuaternion((*rot).quaternion());
    UE_LOG(LogTemp, Warning, TEXT("UTransformReceiver: Received actor (%s) rotation update (%s)"),
...
void UTransformReceiver::Initialise()
{
  if (EntityId == -1)
  {
    ...
    UE_LOG(
        LogTemp, Warning,
        TEXT(
            "UTransformReceiver: Entity id not set to a valid entity id, getting new entity id %s"),
        *ToString(EntityId))
    return;
  }
  ...
  if (!mCallbacks.IsValid())
  {
    ...
    if (entity != nullptr)
    {
      ...
      if (!transform.empty())
      {
        ...
        // seems to skew the rotation of the actor, probably due to the conversions not being
        // accurate
        // yet
